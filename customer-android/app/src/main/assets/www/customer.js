...
window.onAuthCallback = async url => {
  try {
    ...
    if (!accessToken || !refreshToken) {
      const errorDesc = params.get('error_description') || params.get('error') || (params.get('code') ? 'The provider returned an unsupported authorization-code response. Please try again.' : 'Sign-in was cancelled or the provider redirect is not enabled.');
